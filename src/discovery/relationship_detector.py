...
class RelationshipDetector:
    ...
    def detect_relationships(self) -> List[DetectedRelationship]:
        ...
        logger.info(f"Starting optimized relationship detection (strategy: {self.config.strategy})")
    ...
    def _passes_cardinality_check(self, from_col: ColumnInfo, to_col: ColumnInfo) -> bool:
        """
    ...
    def _analyze_candidates_parallel(self, 
                                    candidates: List[RelationshipCandidate],
                                    start_time: float) -> List[DetectedRelationship]:
        """
        ...
        logger.info(f"Starting parallel overlap analysis for {len(candidates)} candidates "
    ...
    def _check_overlap(self, candidate: RelationshipCandidate) -> Optional[DetectedRelationship]:
        """
        ...
        try:
            ...
            logger.debug(f"Overlap check failed for {from_col.full_name} → {to_col.full_name}: {e}")
            return None
    
    def _calculate_overlap_rate(self, 
                                from_col: ColumnInfo, 
                                to_col: ColumnInfo) -> Tuple[float, str]:
        """
    ...
    def _log_statistics(self):
        ...
        logger.info(f"  After type filter: {self.stats['candidates_after_type_filter']}")
        logger.info(f"  After name filter: {self.stats['candidates_after_name_filter']}")
        logger.info(f"  After index filter: {self.stats['candidates_after_index_filter']}")
        logger.info(f"  After cardinality filter: {self.stats['candidates_after_cardinality_filter']}")
        ...
        logger.info(f"  Time building indexes: {self.stats['time_building_indexes']:.2f}s")
...
def detect_relationships(connection_string: str, 
                        discovery_data: Dict,
                        config: Optional[RelationshipDetectionConfig] = None) -> List[Dict]:
    """
